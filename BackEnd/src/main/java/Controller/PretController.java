...
import Service.PretServiceImpl;
...
public class PretController extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      // Validation
      if (pret.getClientId() <= 0
          || pret.getMontantPret() <= 0
          || pret.getDuree() <= 0
          || pret.getTauxAnnuel() <= 0) {
      ...
      if (pret.getMensualite() <= 0) {
        double mensualite =
            pretService.calculerMensualite(
                pret.getMontantPret(), pret.getTauxAnnuel(), pret.getDuree());
