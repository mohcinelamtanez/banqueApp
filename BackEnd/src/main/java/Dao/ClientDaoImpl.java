...
public class ClientDaoImpl implements ClientDao {
  ...
  public boolean ajouterClient(Client client) {
    try (Connection conn = DBConnection.getConnection()) {
      String query =
          "INSERT INTO client (Nom, Prenom, Ville, Cd_postal, Revenue) VALUES (?, ?, ?, ?, ?)";
  ...
  public boolean modifierClient(Client client) {
    try (Connection conn = DBConnection.getConnection()) {
      String query =
          "UPDATE client SET Nom = ?, Prenom = ?, Ville = ?, Cd_postal = ?, Revenue = ? WHERE Id = ?";
      PreparedStatement stmt = conn.prepareStatement(query);
  ...
  public List<Client> rechercherClients(String critere) {
    ...
    try (Connection conn = DBConnection.getConnection()) {
      String query =
          "SELECT * FROM client WHERE Nom LIKE ? OR Prenom LIKE ? OR Ville LIKE ? OR Cd_postal LIKE ?";
      PreparedStatement stmt = conn.prepareStatement(query);
